...
for position in positions:
    ...
    if "Call_Hour" == position['comment']:
        ...
        break
    elif "Put_Hour" == position['comment']:
        position_id = position['ticket']
        flag_side = 'Put'
        target = position['sl']
...
while True:
    ...
    data_candle = mt5.copy_rates_from(symbol, mt5.TIMEFRAME_H1, utc_from_date, 10)
    ...
    tr_percent = round(daily_volatility, 4)/100 if daily_volatility < 1 else 0.01
    sl_percent = 0.01

    if now.time().minute == 55:
    ...
    elif now.date().weekday() not in [5, 6] and 0 < now.time().minute < 55 and position_id in [0, '0', None]:
