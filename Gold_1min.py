...
while True:
    ...
    if last_minute != now.time().minute:
        ...
        if flag_entry:
            if (flag_side == 'Call' and ((rsi.iloc[-2] > 60 and rsi.iloc[-2] > rsi.iloc[-1]) or (rsi.iloc[-2] < 30 and rsi.iloc[-1] < 30)) ) or (flag_side == 'Put' and ((rsi.iloc[-2] < 40 and rsi.iloc[-2] < rsi.iloc[-1]) or (rsi.iloc[-2] > 70 and rsi.iloc[-1] > 70)) ) or (now.date().weekday() == 5 and now.time() > time(hour=21, minute=45)):
        ...
        elif now.date().weekday() not in [5, 6] and time(hour=00, minute=5) < now.time() < time(hour=23, minute=30) and flag_entry == False and rsi.iloc[-1] > 30 and rsi.iloc[-2] < 30: # and rsi.iloc[-3] > 30
        ...
        elif now.date().weekday() not in [5, 6] and time(hour=00, minute=5) < now.time() < time(hour=23, minute=30) and flag_entry == False and rsi.iloc[-1] < 70 and rsi.iloc[-2] > 70: # and rsi.iloc[-3] < 70
